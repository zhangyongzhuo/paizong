...
//                 "事实上是事实是事实", "没有护照", "证件不全", "在逃人员", "扣留口流利", "事实上是事实是事实"]
    ...
    Connections{
        ...
        onMultiPhotosShotFinished:{
            if(receivePageName=="RealisticPage"){
                photoDisplyViewModel.clear()
...
//        }
    ...
    takephoto.onClicked: {
        getFocus.focus = true
        stackView.push({item: "qrc:/singleFunction/ui/MultiPhotosShootPage.qml", properties:{photocount: 10,receivePageName:"RealisticPage"}})
    }
